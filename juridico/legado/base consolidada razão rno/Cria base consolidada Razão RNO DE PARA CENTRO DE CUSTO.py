...
# Transpose rows to columns
df_pivot = df.groupBy("`ID PROCESSO`").pivot("row_num").agg(
    first(col("Valor")).alias("1"),
    first(col("Conta")).alias("2")
)
...
df_razao_bart_conta_1 = df_pivot.select(
    col("ID PROCESSO"),
    col("1_1").alias("CONTA_1"),
    col("1_2").alias("CONTA_2")
)
...
df_razao_bart_conta_2 = df_pivot.select(
    col("ID PROCESSO"),
    col("1_1").alias("VALOR_1"),
    col("1_2").alias("VALOR_2")
)
...
arquivo = f'22.10.2024 Fechamento Trabalhista.xlsx'
