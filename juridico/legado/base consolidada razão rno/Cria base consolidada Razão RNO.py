...
# Converte a coluna CENTRO_DE_CUSTO_AREA_DEMANDANTE_CODIGO de string para double
df_consolidado = df_consolidado.withColumn("CENTRO_DE_CUSTO_AREA_DEMANDANTE_CODIGO", df_consolidado["CENTRO_DE_CUSTO_AREA_DEMANDANTE_CODIGO"].cast(DoubleType()))
...
df_razao_bart_conta.show()
df_razao_bart_conta.printSchema()
...
df_rno_1a = df_rno_1a.orderBy(['ID', 'Centro custo'])
...
# Renomear coluna e ordenar por ID e Centro_Custo
#df_rno_1a = df_rno_1a.withColumnRenamed('Centro_Custo', 'Centro Custo').orderBy(['ID', 'Centro Custo'])

# Remover duplicatas mantendo apenas as primeiras ocorrências de ID e Centro_Custo
df_rno_1b = df_rno_1a.dropDuplicates(['ID', 'Centro custo'])

# Janela para particionar por ID e ordenar pelas linhas
window_spec = Window.partitionBy('ID').orderBy('Centro custo')

# Adicionar a coluna FLAG_CC que conta o número de Centros de Custo por ID
df_rno_1b = df_rno_1b.withColumn('FLAG_CC', row_number().over(window_spec))

# Filtrar apenas a última ocorrência de cada ID (similar ao IF LAST.ID THEN OUTPUT)
df_rno_1c = df_rno_1b.withColumn('max_flag', count('Centro custo').over(window_spec)) \
                                 .filter(col('FLAG_CC') == col('max_flag')) \
                                 .select('ID', 'FLAG_CC')
...
df_rno_1c = df_rno_1c.withColumnRenamed('Centro custo', 'Centro Custo')
...
num_vazios = df_rno_1d.filter(
    (df_rno_1d["Centro de Custo (M)"].isNull()) | (df_rno_1d["Centro de Custo (M)"] == "")
).count()
...
arquivo = f'22.08.2024 Fechamento Trabalhista.xlsx'
...
display(tb_fecham_trab_1a)

# COMMAND ----------

print(tb_fecham_trab_1a.columns)

# COMMAND ----------

print(df_dp_razao_2.columns)

# COMMAND ----------

df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Nº Processo", "No_PROCESSO")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Sub Objeto Assunto/Cargo (M)", "SUB_OBJETO_ASSUNTO_CARGO_M")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("% Empresa (M)", "% EMPRESA (M)")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Objeto Assunto/Cargo (M)", "OBJETO ASSUNTO/CARGO (M)")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("% Sócio (M)", "% SÓCIO (M)")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Vlr Causa", "VLR CAUSA")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Demitido por Reestruturação", "DEMITIDO_POR_REESTRUTURACAO")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Distribuição", "DISTRIBUIÇÃO")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Grupo (M)", "GRUPO (M)")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Centro de Custo (M)", "CENTRO DE CUSTO (M)")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Empresa (M)", "EMPRESA (M)")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Área do Direito", "ÁREA DO DIREITO")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Escritorio", "ESCRITORIO")
df_dp_razao_2 = df_dp_razao_2.withColumnRenamed("Natureza Operacional (M)", "NATUREZA_OPERACIONAL_M")
...
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Nº Processo", "No_PROCESSO")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Sub Objeto Assunto/Cargo (M)", "SUB_OBJETO_ASSUNTO_CARGO_M")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("% Empresa (M)", "% EMPRESA (M)")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Objeto Assunto/Cargo (M)", "OBJETO ASSUNTO/CARGO (M)")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("% Sócio (M)", "% SÓCIO (M)")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Vlr Causa", "VLR CAUSA")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Demitido por Reestruturação", "DEMITIDO_POR_REESTRUTURACAO")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Distribuição", "DISTRIBUIÇÃO")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Grupo (M)", "GRUPO (M)")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Centro de Custo (M)", "CENTRO DE CUSTO (M)")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Empresa (M)", "EMPRESA (M)")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Área do Direito", "ÁREA DO DIREITO")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Escritorio", "ESCRITORIO")
df_dp_rno_1 = df_dp_rno_1.withColumnRenamed("Natureza Operacional (M)", "NATUREZA_OPERACIONAL_M")
...
distinct_df = df_fecham_trab_4.select("TOTAL_CONSOLIDADO_RAZAO").distinct()
display(distinct_df)
...
fecham_trab_razao_rno_1 = fecham_trab_razao_rno \
    ...
    .select(
        ...
        when(fecham_trab_razao_rno['EMPRESA_M'] == 'INDÚSTRIA DE MÓVEIS BARTIRA LTDA', 'BARTIRA').otherwise(df_dp_bu['BU**']).alias('BU'),
        when(fecham_trab_razao_rno['EMPRESA_M'] == 'INDÚSTRIA DE MÓVEIS BARTIRA LTDA', 'BARTIRA').otherwise(df_dp_funcional['nome2']).alias('VP'),
        when(fecham_trab_razao_rno['EMPRESA_M'] == 'INDÚSTRIA DE MÓVEIS BARTIRA LTDA', 'BARTIRA').otherwise(df_dp_funcional['nome3']).alias('DIRETORIA'),
        when(fecham_trab_razao_rno['EMPRESA_M'] == 'INDÚSTRIA DE MÓVEIS BARTIRA LTDA', 'BARTIRA').otherwise(upper(df_dp_funcional['nome responsavel'])).alias('RESPONSAVEL_AREA'),
        when(fecham_trab_razao_rno['EMPRESA_M'] == 'INDÚSTRIA DE MÓVEIS BARTIRA LTDA', 'BART').otherwise(df_dp_funcional['area funcional']).alias('AREA_FUNCIONAL')
    )
...
display(fecham_trab_razao_rno_3)

# COMMAND ----------

from pyspark.sql.functions import desc

# Descrição e contagem do campo AREA_FUNCIONAL
area_funcional_desc = fecham_trab_razao_rno_3.groupBy("AREA_FUNCIONAL").count().orderBy(desc("count"))

display(area_funcional_desc)
...
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_7.select(
    ...
    "`Natureza Operacional (M)`", "`Média de Pagamento`", "`% Risco`", "Distribuicao", "`No_PROCESSO`", 
    "ESCRITORIO", "`Vlr Causa`", "`%_Socio_M_1`", "`%_Empresa_M_1`", "`%_Socio_M`", "`%_Empresa_M`", 
    ...
    "`EMPRESA:_Correcao_Mov_M`", "`EMPRESA:_Provisao_Mov_Total_M`", "`EMPRESA: Provisão Total (M)`", 
...
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Centro de Custo (M)", col("Centro de Custo (M)").cast("int"))
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Centro de Custo (M-1)", col("Centro de Custo (M-1)").cast("int"))

# COMMAND ----------

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Centro de Custo (M)", col("Centro de Custo (M)").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Centro de Custo (M-1)", col("Centro de Custo (M-1)").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("ACORDO_RAZAO", col("ACORDO_RAZAO").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("CNOVA_RAZAO", col("CNOVA_RAZAO").cast("double"))


fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("CREDITOS_RAZAO", col("CREDITOS_RAZAO").cast("double"))



fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("TOTAL_DESPESA_RAZAO", col("TOTAL_DESPESA_RAZAO").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("TOTAL_CREDITO_RAZAO", col("TOTAL_CREDITO_RAZAO").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("TOTAL_CREDITO_RAZAO", col("TOTAL_CREDITO_RAZAO").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("TOTAL_CONSOLIDADO_RAZAO", col("TOTAL_CONSOLIDADO_RAZAO").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("CONTA_1", col("CONTA_1").cast("double"))


fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("VALOR_CONTA_1", col("VALOR_CONTA_1").cast("double"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("VALOR_CONTA_2", col("VALOR_CONTA_2").cast("double"))
...
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Provisão Total Passivo (M)", col("Provisão Total Passivo (M)").cast("float"))
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Socio: Provisão (M-1)", col("`Socio: Provisão (M-1)`").cast("float"))
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8 .withColumn("Socio: Correção (M-1)", col("`Socio: Correção (M-1)`").cast("float"))
fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Socio: Provisão Total (M-1)", col("Socio: Provisão Total (M-1)").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Provisão Mov. (M)", col("`SOCIO: Provisão Mov. (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Correção Mov. (M)", col("`SOCIO: Correção Mov. (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Provisão Mov. Total (M)", col("`SOCIO: Provisão Mov. Total (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Provisão Total (M)", col("`SOCIO: Provisão Total (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Correção (M-1)_0001", col("`SOCIO: Correção (M-1)_0001`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Correção (M)", col("`SOCIO: Correção (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("SOCIO: Prov. Total Passivo (M)", col("`SOCIO: Prov. Total Passivo (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Empresa: Provisão (M-1)", col("`Empresa: Provisão (M-1)`").cast("float"))

fecham_trab_razao_rno_8 =fecham_trab_razao_rno_8.withColumn("Empresa: Correção (M-1)", col("`Empresa: Correção (M-1)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("Empresa: Provisão Total (M-1)", col("`Empresa: Provisão Total (M-1)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA: Classificação Mov.(M)", col("`EMPRESA: Classificação Mov.(M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA: Provisão Mov. (M)", col("`EMPRESA: Provisão Mov. (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA:_Correcao_Mov_M", col("`EMPRESA:_Correcao_Mov_M`").cast("float"))

fecham_trab_razao_rno_8= fecham_trab_razao_rno_8.withColumn("EMPRESA: Provisão Mov. (M)", col("`EMPRESA: Provisão Mov. (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA: Provisão Total (M)", col("`EMPRESA: Provisão Total (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA: Correção (M-1)_0001", col("`EMPRESA: Correção (M-1)_0001`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA: Correção (M)", col("`EMPRESA: Correção (M)`").cast("float"))

fecham_trab_razao_rno_8 = fecham_trab_razao_rno_8.withColumn("EMPRESA: Prov Total Passivo(M)", col("`EMPRESA: Prov Total Passivo(M)`").cast("float"))


# COMMAND ----------

display(fecham_trab_razao_rno_8)
...
# Descrição e contagem do campo AREA_FUNCIONAL
area_funcional_desc = fecham_trab_razao_rno_8.groupBy("VALOR_CONTA_1").count().orderBy(desc("count"))
...
# # Certifique-se de que você está usando uma versão do Spark que suporta o parâmetro allowMissingColumns
# # Este recurso requer Apache Spark 3.1.0 ou superior
# from pyspark.sql.functions import *

# for column in df_hist_razao_rno_fin.columns:
#     adjust_column = column.replace('.','')
#     df_hist_razao_rno_fin = df_hist_razao_rno_fin.withColumnRenamed(column, adjust_column)
#     df_hist_razao_rno_fin = df_hist_razao_rno_fin.withColumn(cols, col(adjust_column).cast("string"))


# for column in fecham_trab_razao_rno_7.columns:
#     adjust_column = column.replace('.','')
#     fecham_trab_razao_rno_7 = fecham_trab_razao_rno_7.withColumnRenamed(column, adjust_column)
#     fecham_trab_razao_rno_7 = fecham_trab_razao_rno_7.withColumn(cols, col(adjust_column).cast("string"))

# df_union = df_hist_razao_rno_fin.union(fecham_trab_razao_rno_7)

# # Para visualizar o DataFrame resultante
# # display(df_union)
...
pandas_df = fecham_trab_razao_rno_8.toPandas()
...
path_df_hist = '/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_rno/FECHAM_TRAB_RAZAO_RNO_202407.xlsx'

df_rno_hist_2224 = read_excel(path_df_hist,'A1')

# COMMAND ----------

df_fech_trab_ago_24 = '/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_rno/df_fech_trab_razao_rno_08_MES.xlsx'

df_fech_trab_ago_24_final= read_excel(df_fech_trab_ago_24,'A1')

# COMMAND ----------

from pyspark.sql.functions import col

df_rno_hist_2224 = df_rno_hist_2224.filter(col("MES_FECH") > "2023-01-01")

# COMMAND ----------

df_rno_hist_2224.count()

# COMMAND ----------

FECHAM_TRAB_RAZAO_RNO_2023A202408 = df_rno_hist_2224.unionByName(df_fech_trab_ago_24_final)
display(FECHAM_TRAB_RAZAO_RNO_2023A202408)
...
# Converter PySpark DataFrame para Pandas DataFrame
pandas_df = df_hist.toPandas()

# Salvar o DataFrame com os nomes corrigidos em um arquivo Excel no disco local primeiro
local_path = f'/local_disk0/tmp/df_hist_/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_rno/df_fech_trab_razao_rno_08_FIT.xlsx{nmmes}.xlsx'
pandas_df.to_excel(local_path, index=False, sheet_name='FECH_RAZAO_RNO')

# Copiar o arquivo do disco local para o volume desejado
volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_rno/df_hist_{nmmes}.xlsx'
