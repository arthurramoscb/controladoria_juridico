...
base_provisao = f'/Volumes/databox/juridico_comum/arquivos/financeiro/bases de provisão/Input/{tipo_arquvivo} Procon_{nm_base}.xlsx'
...
df_base_provisao = read_excel(base_provisao, "'Final'!A4:BK1048576")
...
WHERE `ÁREA DO DIREITO` IN ('PROCON')
...
AND `SUB TIPO` IN ('ACORDO - REGULATÓRIO - PROCON COM AUDIÊNCIA','ACORDO/ PROGRAMAS DE PARCELAMENTO - REGULATÓRIO','MULTA - REGULATÓRIO','ACORDO - MEDIAÇÃO')
""")
...
SELECT *
,(CASE WHEN `SUB TIPO` IN ('MULTA - REGULATÓRIO') THEN 'Condenacao' 

    WHEN `SUB TIPO` IN ('ACORDO - REGULATÓRIO - PROCON COM AUDIÊNCIA','ACORDO/ PROGRAMAS DE PARCELAMENTO - REGULATÓRIO','ACORDO - MEDIAÇÃO') THEN 'Acordo' END) AS TIPO_PGTO
...
WHERE `ÁREA DO DIREITO` IN ('PROCON') 
AND `STATUS DO PAGAMENTO` NOT IN ('CANCELADO', 'EM CORREÇÃO', 'PAGAMENTO', 'DEVOLVIDO','PAGAMENTO DEVOLVIDO', 'Pendente', 'REJEITADO', 'Em Levantamento') AND `SUB TIPO` NOT IN ('CUSTAS PROCESSUAIS - REGULATÓRIO')
""")
...
SELECT *
,(CASE 
    WHEN `SUB TIPO` IN ('ACORDO - REGULATÓRIO - PROCON COM AUDIÊNCIA','ACORDO/ PROGRAMAS DE PARCELAMENTO - REGULATÓRIO','ACORDO - MEDIAÇÃO') THEN 'Acordo'
    ELSE 'Condenacao' 
END) AS TIPO_PGTO
...
SELECT *
,(CASE WHEN `ÁREA DO DIREITO` IN ('PROCON') THEN 'Condenacao' END) AS TIPO_PGTO
...
local_path = f'/local_disk0/tmp/Base_consolidada_procon.xlsx'
pandas_df.to_excel(local_path, index=False, sheet_name='BASE')
# Copiar o arquivo do disco local para o volume desejado
volume_path = f'/Volumes/databox/juridico_comum/arquivos/financeiro/bases de provisão/output/Base_consolidada_procon.xlsx'
